  ...
  return (
    <section id="problema" className="bg-white py-14 sm:py-20 lg:py-28">
      <div className="mx-auto max-w-3xl px-5">
        <h2 className="text-xl sm:text-2xl lg:text-4xl font-bold text-[#0B2A4A] tracking-tight leading-snug">
          Si tu negocio depende de la tecnología, una de estas cosas te ha
          pasado en el último año.
        </h2>

        <ol className="mt-8 sm:mt-10 space-y-6 sm:space-y-7">
          {items.map((item, i) => (
            <li key={i} className="flex gap-4 sm:gap-5">
              <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 bg-[#0B2A4A] text-white font-bold flex items-center justify-center text-xs sm:text-sm">
                {i + 1}
              </div>
              <div>
                ...
                <p className="mt-1 text-sm sm:text-base text-[#4d4d4d] leading-relaxed">
                  {item.text}
                </p>
              </div>
            </li>
